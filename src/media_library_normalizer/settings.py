...
class Settings:
    ...
    def from_env(cls) -> Settings:
        ...
        """
        workspace_path: Path = Path(os.getenv("MLN_WORKSPACE_PATH", "./data/workspace"))

        return cls(
            app_name=os.getenv("MLN_APP_NAME", "media-library-normalizer"),
            library_path=Path(os.getenv("MLN_LIBRARY_PATH", "./data/library")),
            workspace_path=workspace_path,
            cache_path=Path(os.getenv("MLN_CACHE_PATH", str(workspace_path / "cache"))),
            reports_path=Path(os.getenv("MLN_REPORTS_PATH", str(workspace_path / "reports"))),
            manifests_path=Path(os.getenv("MLN_MANIFESTS_PATH", str(workspace_path / "manifests"))),
            logs_path=Path(os.getenv("MLN_LOGS_PATH", str(workspace_path / "logs"))),
            log_level=os.getenv("MLN_LOG_LEVEL", "INFO").upper(),
            log_format=os.getenv("MLN_LOG_FORMAT", "text").lower(),
            dry_run=_parse_bool(os.getenv("MLN_DRY_RUN", "true")),
            tmdb_api_key=_optional_env("MLN_TMDB_API_KEY"),
            tvdb_api_key=_optional_env("MLN_TVDB_API_KEY"),
            provider_lookup_progress_interval=int(os.getenv("MLN_PROVIDER_LOOKUP_PROGRESS_INTERVAL", "100")),
        )
